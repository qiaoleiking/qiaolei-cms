	 ...
	 <form action="" id="form">
		...
		<div class="form-group row ">
			<label for="title">文章标题</label> <input type="text"
				class="form-control" id="title" name="title" value="${articel.title}"  placeholder="请输入标题">
		</div>
		...
		<div class="form-group row ">
			...
			<select class="custom-select custom-select-sm mb-3" id="channel"  name="channel_id">
			  <option value="0">请选择</option>
			  <c:forEach items="${channelList}" var="channel">
			  		<c:choose>
			  			<c:when test="${channel.id == articel.channel.id}">
		...
		<div class="form-group row ">
			...
			<select class="custom-select custom-select-sm mb-3" id="category" name="category_id">
			</select>
		</div>
...
// 当频道的数据发生改变的时候，分类需要联动
 ...
 function changeChannel(){
	   ...
	   // 根据频道的数据获取相应的分裂
	   $.post("/article/getCategoryByChannel",{chaId:channelId},function(data){
		   //data 包含了分类的信息
		   if(data.result==1){
			   // 清空数据
			   $("#category").empty();
			   ...
			   $("#category").append("<option value='0'>请选择</option>")
				   for ( var index in data.data) {
					   if(data.data[index].id ==  "${articel.category_id}"  ){
